...
def login():
    if request.method == 'POST':
        ...
        print(f'Intentando iniciar sesiín con usuario: {username} y contraseña: {password}')
        exito = sistema.login(username, password)
        if exito:
            session['logged_in'] = True
            session['username'] = sistema.usuario_actual
            flash('Inicio de sesión exitoso', 'succes')
